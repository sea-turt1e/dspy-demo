"""
Part 3: MIPROv2 による自動最適化
================================
MIPROv2 を使って、プロンプトを自動的に最適化します。
手動でプロンプトを書き換えることなく、正答率を向上させます。

MIPROv2 の仕組み（3ステップ）:
  1. Bootstrap Few-Shot Examples
     → 訓練データから成功した例を few-shot 候補として収集
  2. Propose Instruction Candidates
     → LLM を使って高品質な命令文（プロンプト）を複数生成
  3. Bayesian Optimization
     → 命令文と few-shot 例の最適な組み合わせをベイズ最適化で探索

実行方法:
  uv run python 03_optimize.py
"""

import os

import dspy
from dotenv import load_dotenv
from dspy.datasets.hotpotqa import HotPotQA
from dspy.evaluate import Evaluate
from dspy.evaluate.metrics import answer_exact_match
from dspy.teleprompt import MIPROv2

# override=True: .zshrc 等で設定済みの環境変数よりも .env の値を優先する
load_dotenv("../../.env", override=True)


def main():
    print("=" * 60)
    print("Part 3: MIPROv2 による自動最適化")
    print("=" * 60)

    # ============================================================
    # 1. 言語モデルとデータセットの設定
    # ============================================================
    lm_model = os.getenv("OPENAI_MODEL", "openai/gpt-5-nano")
    lm = dspy.LM(lm_model)
    dspy.configure(lm=lm)
    print(f"\n✅ 言語モデルを設定しました: {lm_model}")

    hotpotqa = HotPotQA(
        train_seed=1,
        train_size=150,
        eval_seed=2023,
        dev_size=50,
    )
    trainset = [x.with_inputs("question") for x in hotpotqa.train]
    devset = [x.with_inputs("question") for x in hotpotqa.dev]
    print(f"✅ HotPotQA データセットを読み込みました（訓練: {len(trainset)}, 評価: {len(devset)}）")

    # ============================================================
    # 2. ベースラインプログラムの定義
    # ============================================================
    # Predict（推論過程なし）をベースラインとして使います。
    # MIPROv2 が命令文と few-shot 例を追加して、どれだけ改善できるか見ます。
    print("\n" + "-" * 60)
    print("🔹 ベースラインの正答率を測定中...")
    print("-" * 60)

    baseline = dspy.Predict("question -> answer")

    evaluator = Evaluate(
        devset=devset,
        metric=answer_exact_match,
        num_threads=4,
        display_progress=True,
    )

    # evaluator() は EvaluationResult オブジェクトを返すので .score で数値を取得
    baseline_result = evaluator(baseline)
    baseline_score = baseline_result.score
    print(f"\n📊 ベースライン正答率: {baseline_score:.1f}%")

    # ============================================================
    # 3. MIPROv2 の設定と実行
    # ============================================================
    print("\n" + "-" * 60)
    print("🔹 MIPROv2 による最適化を開始します")
    print("-" * 60)

    # MIPROv2 のインスタンスを作成
    # auto パラメータで最適化の強度を設定:
    #   "light"  → 少ない試行回数で高速（デモ向き）
    #   "medium" → バランスの取れた設定
    #   "heavy"  → 多くの試行回数で高精度（本番向き）
    # auto=None にして num_trials 等を手動で設定
    # auto="light" のデフォルトだと trial が多すぎるため、デモ用に絞る
    teleprompter = MIPROv2(
        metric=answer_exact_match,    # 評価関数
        auto=None,                    # 手動設定モード
        num_candidates=3,             # 命令文の候補数
        max_bootstrapped_demos=4,     # Bootstrap few-shot 例の最大数
        max_labeled_demos=4,          # ラベル付き few-shot 例の最大数
    )

    print("\n  最適化設定:")
    print("    - metric: answer_exact_match（回答の完全一致）")
    print("    - num_trials: 5（試行回数）")
    print("    - num_candidates: 3（命令文の候補数）")
    print("\n  🚀 最適化を実行中...（数分かかる場合があります）")

    # compile() で最適化を実行
    # trainset を使って最適なプロンプトと few-shot 例を探索します
    optimized_program = teleprompter.compile(
        baseline,              # 最適化するプログラム
        trainset=trainset,     # 訓練データ
        num_trials=5,          # 試行回数（デモなので少なめに設定）
    )

    print("\n✅ 最適化が完了しました！")

    # ============================================================
    # 4. 最適化後の正答率を測定
    # ============================================================
    print("\n" + "-" * 60)
    print("🔹 最適化後の正答率を測定中...")
    print("-" * 60)

    optimized_result = evaluator(optimized_program)
    optimized_score = optimized_result.score
    print(f"\n📊 最適化後の正答率: {optimized_score:.1f}%")

    # ============================================================
    # 5. Before / After の比較
    # ============================================================
    print("\n" + "-" * 60)
    print("🔹 Before / After 比較")
    print("-" * 60)

    improvement = optimized_score - baseline_score
    print(f"""
    ┌─────────────────────────────────────┐
    │  ベースライン正答率:  {baseline_score:>6.1f}%       │
    │  最適化後の正答率:    {optimized_score:>6.1f}%       │
    │  改善幅:              {improvement:>+6.1f}%       │
    └─────────────────────────────────────┘
""")

    # ============================================================
    # 6. 最適化済みプログラムの保存
    # ============================================================
    print("-" * 60)
    print("🔹 最適化済みプログラムを保存")
    print("-" * 60)

    # JSON ファイルとして保存。後で読み込んで再利用できます。
    save_path = "optimized_hotpotqa.json"
    optimized_program.save(save_path)
    print(f"\n✅ 保存しました: {save_path}")

    # ============================================================
    # 7. 最適化されたプロンプトの中身を確認
    # ============================================================
    print("\n" + "-" * 60)
    print("🔹 最適化されたプロンプトの中身を確認")
    print("-" * 60)

    # MIPROv2 が自動生成した命令文を確認
    for i, predictor in enumerate(optimized_program.predictors()):
        print(f"\n  Predictor {i}:")
        if hasattr(predictor, "extended_signature"):
            sig = predictor.extended_signature
            if sig.instructions:
                print(f"    命令文: {sig.instructions[:200]}...")
        if hasattr(predictor, "demos") and predictor.demos:
            print(f"    Few-shot 例の数: {len(predictor.demos)}")

    # ============================================================
    # まとめ
    # ============================================================
    print("\n" + "=" * 60)
    print("📌 Part 3 まとめ")
    print("=" * 60)
    print(f"""
MIPROv2 による最適化の結果:
  ベースライン: {baseline_score}% → 最適化後: {optimized_score}% ({improvement:+.1f}%)

MIPROv2 がやったこと:
  1. 訓練データから成功した few-shot 例を収集
  2. 高品質な命令文（プロンプト）を自動生成
  3. ベイズ最適化で最適な組み合わせを探索

ポイント:
  - プロンプトを1文字も手書きしていない！
  - DSPy が自動的に最適なプロンプトを発見
  - optimized_hotpotqa.json に保存 → いつでも再利用可能

次のステップ:
  uv run python 04_inference.py
""")


if __name__ == "__main__":
    main()
